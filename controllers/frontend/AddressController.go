...
func (c *AddressController) AddAddress() {
	...
	models.DB.Table("address").Where("uid=?", user.Id).Update(map[string]interface{}{"default_address": 0})
...
func (c *AddressController) ChangeDefaultAddress() {
	...
	models.DB.Table("address").Where("uid=?", user.Id).Update(map[string]interface{}{"default_address": 0})
	models.DB.Table("address").Where("id=?", addressId).Update(map[string]interface{}{"default_address": 1})
	c.Data["json"] = map[string]interface{}{
		"success": true,
		"result":  "更新默认收货地址成功",
	}
