...
func (c *PayController) Alipay() {
	...
	var privateKey = "xxxxxxx" //必须，上一步使用RSA签名验签工具生成的私钥
	...
	client.LoadAliPayRootCertFromFile("certfile/alipayRootCert.certfile")
	client.LoadAliPayPublicCertFromFile("certfile/alipayCertPublicKey_RSA2.certfile")
...
func (c *PayController) AlipayNotify() {
	var privateKey = "xxxxxxxxxxxxxxx" //必须,上一步中使用RSA签名验签工具 生成的私钥
	var client, err = alipay.New("2021001186696588", privateKey, true)
	client.LoadAppPublicCertFromFile("certfile/appCertPublicKey_2021001186696588.certfile") //加载应用公钥证书
	client.LoadAliPayRootCertFromFile("certfile/alipayRootCert.certfile")
	client.LoadAliPayPublicCertFromFile("certfile/alipayCertPublicKey_RSA2.certfile")
	...
	alipay.AckNotification(rep) //确认收到的通知消息
}
...
func (c *PayController) WxPay() {
	...
	params.SetString("body", "order--"+time.Now().Format(template)).
		...
		SetString("notify_url", "http://xxxxxx/wxpay/notify"). //配置的回溯地址
		//SetString("trade_type","APP") //APP端支付
		SetString("trade_type", "NATIVE") //网站支付
...
func (c *PayController) WxPayNotify() {
	//获取表单传过来的xml数据,在配置文件里设置copyrequestbody =true
